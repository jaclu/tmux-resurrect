...
supported_tmux_version_ok() {
	my_log supported_tmux_version_ok
	$CURRENT_DIR/check_tmux_version.sh "$SUPPORTED_VERSION"
}

remove_first_char() {
	my_log remove_first_char
	echo "$1" | cut -c2-
}

capture_pane_contents_option_on() {
	my_log capture_pane_contents_option_on
...
pane_contents_create_archive() {
	my_log pane_contents_create_archive
	tar cf - -C "$(resurrect_dir)/save/" ./pane_contents/ |
		gzip > "$(pane_contents_archive_file)"
}

pane_content_files_restore_from_archive() {
	my_log pane_content_files_restore_from_archive
...
last_resurrect_file() {
	my_log last_resurrect_file
	echo "$(resurrect_dir)/last"
}

pane_contents_dir() {
	my_log pane_contents_dir
	echo "$(resurrect_dir)/$1/pane_contents/"
}

pane_contents_file() {
	my_log pane_contents_file
...
pane_contents_file_exists() {
	my_log pane_contents_file_exists
	local pane_id="$1"
	[ -f "$(pane_contents_file "restore" "$pane_id")" ]
}

pane_contents_archive_file() {
	my_log pane_contents_archive_file
